...
public class FighterAI : MonoBehaviour
{
    ...
    void Update() {
        ...
        switch (currentState) {
            ...
            case AIState.Chase:
                ...
                if (distance <= attackRange * 1.4f) {  // Increased attack range
            ...
            case AIState.Attack:
                ...
                if (distance > attackRange * 1.8f) {  // More lenient distance for staying in attack
                    currentState = AIState.Chase;
                } else {
                    Attack();
    ...
    void MoveTowardsPlayer() {
        ...
        if ((player.position.y > transform.position.y + 0.2f && distance < attackRange * 2.5f) || // Jump when player is slightly above
            ...
            (Mathf.Abs(player.position.x - transform.position.x) < 0.8f && Random.value < 0.15f)) { // More aggressive dodge jumps
    ...
    void Attack() {
        ...
        if (distance <= attackRange * 1.4f) {  // Increased attack range
            ...
            if (canMainAttack && canKick) {
                ...
                if (randomValue < kickProb) {  // Independent chance for kick
                    Kick();
                    if (shouldTryCombo) {
